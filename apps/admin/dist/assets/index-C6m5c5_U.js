...
`):'<span class="muted">No metrics yet</span>'}function A(e){const t=e.filter(s=>typeof s=="number"&&Number.isFinite(s));return t.length===0?0:t.reduce((s,l)=>s+l,0)/t.length}async function c(e,t={}){const s=await fetch(`/api${e}`,{credentials:"include",headers:{"Content-Type":"application/json",...t.headers||{}},...t});if(!s.ok)throw new Error(await s.text());return s.json()}function L(e){n.activeSection=e,document.querySelectorAll(".nav-item").forEach(t=>{t.classList.toggle("active",t.dataset.section===e)}),document.querySelectorAll(".section").forEach(t=>{t.classList.toggle("active",t.id===`section-${e}`)}),document.getElementById("section-title").textContent=b[e][0],document.getElementById("section-subtitle").textContent=b[e][1]}async function S(){try{const e=await c("/auth/me");n.operator=e,document.getElementById("operator-label").textContent=`signed in as ${e.username}`,f.classList.add("hidden"),p.classList.remove("hidden"),await N()}catch{f.classList.remove("hidden"),p.classList.add("hidden")}}async function N(){await Promise.all([I(),y()])}function C(){const e=document.getElementById("agent-search").value.trim(),t=document.getElementById("agent-status-filter").value,s=new URLSearchParams;return e&&s.set("search",e),t&&s.set("status",t),s.set("limit","50"),s.toString()}async function I(){var l;const e=C(),t=await c(`/admin/agents${e?`?${e}`:""}`);n.agents=t.items||[],n.agentDetails.clear();const s=await Promise.all(n.agents.map(a=>c(`/admin/agents/${a.id}`)));s.forEach(a=>n.agentDetails.set(a.id,a)),(!n.selectedAgentId||!n.agentDetails.has(n.selectedAgentId))&&(n.selectedAgentId=((l=s[0])==null?void 0:l.id)??null),n.selectedAgentId&&await k(n.selectedAgentId),P()}async function k(e){n.agentTokens=(await c(`/admin/agents/${e}/tokens`)).items||[],$()}async function y(){n.enrollmentTokens=(await c("/admin/enrollment-tokens")).items||[],D()}function P(){const e=n.agents.length,t=n.agents.filter(d=>d.status==="online").length,s=e-t,l=A(Array.from(n.agentDetails.values()).map(d=>{var i,h;return(h=(i=d.latestHeartbeat)==null?void 0:i.system)==null?void 0:h.cpuPercent})),a=A(Array.from(n.agentDetails.values()).map(d=>{var h;const i=(h=d.latestHeartbeat)==null?void 0:h.system;return!i||!i.memoryTotalBytes?null:i.memoryUsedBytes/i.memoryTotalBytes*100}));document.getElementById("fleet-kpis").innerHTML=[["Agents",e],["Online",t],["Offline",s],["Avg CPU",`${u(l,1)}%`],["Avg Mem",`${u(a,1)}%`]].map(([d,i])=>`<div class="kpi"><span class="muted">${d}</span><strong>${i}</strong></div>`).join("");const r=Array.from(n.agentDetails.values());document.getElementById("agents-output").innerHTML=r.length?`
      <table>
        ...
        <tbody>
          ${r.map(d=>{var i;return`
              <tr>
                <td><button class="agent-select" data-agent-id="${d.id}"><code>${o(d.hostId)}</code></button></td>
                <td class="${d.status==="online"?"status-online":"status-offline"}">${d.status}</td>
                <td>${o(d.hostname)}</td>
                <td>${o(d.os)}</td>
                <td>${o(g(d.lastSeenAt))}</td>
                <td class="metrics">${v((i=d.latestHeartbeat)==null?void 0:i.system)}</td>
              </tr>`}).join("")}
        </tbody>
      </table>`:'<div class="empty">No agents found.</div>',document.querySelectorAll("[data-agent-id]").forEach(d=>{d.onclick=async()=>{n.selectedAgentId=d.dataset.agentId,await k(n.selectedAgentId),w(),O()}}),w()}function w(){var t;const e=n.agentDetails.get(n.selectedAgentId);if(!e){document.getElementById("agent-detail").innerHTML='<div class="empty">Select an agent to inspect details.</div>',$();return}document.getElementById("drawer-title").textContent=`${e.hostId} details`,document.getElementById("drawer-subtitle").textContent=`${e.hostname} · ${e.os}`,document.getElementById("agent-detail").innerHTML=`
    <div class="detail-grid">
      <div class="detail-card"><span class="muted">Agent ID</span><code>${o(e.id)}</code></div>
      <div class="detail-card"><span class="muted">Installation ID</span><code>${o(e.installationId)}</code></div>
      <div class="detail-card"><span class="muted">Last seen</span><strong>${o(g(e.lastSeenAt))}</strong></div>
    ...
    <div class="panel top-gap">
      ...
      <pre class="metrics">${o(v((t=e.latestHeartbeat)==null?void 0:t.system).replace(/<[^>]+>/g,""))}</pre>
    </div>
    <div class="panel top-gap">
      ...
      <table>
        ...
        <tbody>
          ${(e.recentHeartbeats||[]).map(s=>`
              <tr>
                <td>${o(g(s.receivedAt))}</td>
                <td>${o(s.health)}</td>
                <td>${o(String(s.queueDepth))}</td>
      ...
      <table>
        ...
        <tbody>
          ${n.logs.map(e=>`
              <tr>
                <td>${o(g(e.timestamp))}</td>
                <td><code>${o(e.hostId)}</code></td>
                <td>${o(e.sourceType)}</td>
                <td>${o(e.level||"—")}</td>
                <td>
                  <div>${o(e.message)}</div>
                  <details>
                    <summary class="muted">event details</summary>
                    <pre>${o(JSON.stringify(e,null,2))}</pre>
                  </details>
                </td>
              </tr>`).join("")}
        </tbody>
